...
- Кнопки на input с подтягивающим резистором
- Кнопки на input с подтягивающим резистором
- Кнопки на input с подтягивающим резистором

**Турель с капсулами + кофемашина**
- Магнит для вкл/выкл(Реле)
- Сенсор на землю(Реле) для переключения программы
- Сервопривод
- Сервопривод
- Концевик
- Концевик(в целом резерв)
- Шаговый двигатель(+ драйвер)
- Ir датчик присутствия капсулы
- Механический(оптический) концевик для фиксации завершения приготовления 
...
**Манипулятор для подачи стакана**
- Шаговый двигатель
- Концевик
- Датчик ИК
- Сервопривод
...
- [x] Разработка ПО для микроконтроллера(для тестового стенда), выполнение полного цикла приготовления кофе на тестовом стенде в ручную с компа посредством ручного ввода 
- [x] Разработка Flask приложения(для тестового стенда), выполнение полного цикла приготовления кофе, оплаты, загрузки, ребута, частично реализовать систему обслуживания
...
- [x] Админ панель(обслуживание) или может пока обойтись кнопкой
- [ ] Выход в сеть, легкий удаленный доступ, объединение в одну сеть(хз проще всего наверное VPN)

### Разработка Flask приложения 
- [x] Создание мини БД для учета(почти готово, надо сделать сохранение после перезапуска)
- [x] Роутинг страниц
- [x] Шаблоны для отображения(черновые, пока без дизайна)
- [x] Подумать как будет храниться информация в БД, пока это только объект внутри программы, возможно можно сделать тупо хранение на флешке файлов JSON
- [x] Код для передачи g-code комманд MK 
- [x] Дизайн всех страниц, сверстать, сделать шаблоны 
- [x] Отрефакторить предыдущие наработки, чтоб все было модульно и круто
- [x] Добавить описание как это все запускать, добавить описание к ['репозиторию']:(https://github.com/1deam0nster/Python_dev) Python_dev
- [x] Создать админ панель(обслуживание автомата для начала), отображение статистики продаж
